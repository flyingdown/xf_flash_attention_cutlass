...
path_to_so = 'build/libpaged-attention.so'
# 动态加载.so文件
spec = importlib.util.spec_from_file_location("paged_attn", path_to_so)
paged_attn = importlib.util.module_from_spec(spec)
spec.loader.exec_module(paged_attn)
...
def flash_attn_func(
    ...
    out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = paged_attn.fwd(
        q,
        k,
        v,
        None,
...
def flash_attn_varlen_func(
    ...
    maybe_contiguous = lambda x: x.contiguous() if x.stride(-1) != 1 else x
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    out, q, k, v, out_padded, softmax_lse, S_dmask, rng_state = paged_attn.varlen_fwd(
        q,
        k,
        v,
        None,
...
def flash_attn_with_kvcache(
    ...
    out, softmax_lse = paged_attn.fwd_kvcache(
        q,
        k_cache,
        v_cache,
        k,
...
# @pytest.mark.parametrize("d", [32, 64, 96, 128, 160, 192, 224, 256])
@pytest.mark.parametrize('d', [40, 64, 80, 128])
@pytest.mark.parametrize(
    "seqlen_q,seqlen_k",
    [
        (1, 147),
...
@pytest.mark.parametrize("paged_kv_block_size", [32])
...
# @pytest.mark.parametrize('seqlen_q,seqlen_k', [(3, 1024)])
